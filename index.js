...
reviews.push({name:'Charles',rating:5,feedback:'Wow, what a great place!'})
...
*/
 ...
 function getReviewByRating(reviewSet, ratingInput) {
  let newArray = []  
  
  for (let i = 0; i < reviewSet.length; i++) {
      if (reviewSet[i].rating >= ratingInput && reviewSet[i].rating < ratingInput+1) {
        newArray.push(reviewSet[i])
      }
    }
  return newArray;
  }
...
function getLongReviews(reviewSet) {
    let newArray = []

    for (let i = 0; i < reviewSet.length; i++) {
      if (reviewSet[i].feedback.split(' ').length > 15) {
        newArray.push(reviewSet[i])
      }
    }
    return newArray
